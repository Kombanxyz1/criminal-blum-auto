...
░▒█▄▄▀░▒█░▒█░▄█▄░▒█░░▒█░▄█▄░▒█░░▀█▒█░▒█░▒█▄▄█
    ...
    Blum  Auto Claim Bot by - @XYZ_CRIMINAL
...
{green}total proxy :{white} {len(proxies)}
    ...
    {green}5{white}.{green}) {white}start bot (auto work)
    {green}6{white}.{green}) {white}start bot (sync system)
    {green}7{white}.{green}) {yellow}add proxy
    {green}8{white}.{green}) {yellow}add query
    {green}9{white}.{green}) {red}reset query
    {green}10{white}.{green}) {red}reset proxy
        """
        opt = None
        if args.action:
            opt = args.action
